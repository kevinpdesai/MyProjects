...
// Variable used to calculate the total number of tokens (Words).
long nWords = 0;

// Variables used to calculate the distinct count of tokens and stems.
int distinctCountTokens = 0, distinctCountStems = 0;
...
// Vector to keep the sorted dictionary of tokens and stems.
vector<pair<string,int> > sortedTokens;
vector<pair<string,int> > sortedStems;

// Sets used to store tokens and stems in each file and hence used to assist in calculating the distinct tokens and stems in each file.
...
// Add token in the token dictionary.
void addToken(string word) {
	if(tokens[word] == 0)
		tokens[word] = 1;
	else
		tokens[word]++;
	fileTokens.insert(word);
}
...
void sortDictionaries() {
	copy(tokens.begin(), tokens.end(), back_inserter(sortedTokens));
	sort(sortedTokens.begin(), sortedTokens.end(), cmp);
...
void parseDirectory(char dirName[]) {
	...
	for(int i=0; i<files.size(); i++) {
		...
		fileStems.clear();
		parseFile(fn);
		distinctCountTokens+=fileTokens.size();
...
void printResults() {
	cout << " Number of tokens = " << nWords;
	cout << "\n Number of unique tokens = " << tokens.size();
	cout << "\n Number of tokens that occur only once = " << oneOccurence(tokens);
	cout << "\n Average number of distinct tokens = " << distinctCountTokens/(float)(files.size() - 2);
	cout << "\n Top 30 tokens : \n";
	getTop(sortedTokens,30);
