...
static char *_end;                  /* pointer to the end of the word */
...
EndsWithCVC( char *word )
   ...
   if ( (length = strlen(word)) <= 2 )
	  ...
	  {
	  _end = word + length - 1;
	  return(    (NULL == strchr("aeiouwxy",*_end--))      /* consonant */
			  && (NULL != strchr("aeiouy",  *_end--))        /* vowel */
			  && (NULL == strchr("aeiou",   *_end  )) );   /* consonant */
	  }
...
/*   char *word; in/out: the word stemmed */
   ...
   for ( _end = word; *_end != EOS; _end++ )
	  if ( !isalpha(*_end) ) return( '\0' );
	  else *_end = tolower( *_end );
   _end--;
